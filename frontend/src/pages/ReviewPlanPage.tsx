...
export function ReviewPlanPage({ app }: { app: AppContextValue }) {
  ...
  const grouped = Array.from(new Set(sessions.map((session) => session.startAt.slice(0, 10))))
    .sort()
    .map((day) => ({ day, sessions: sessions.filter((session) => session.startAt.startsWith(day)) }));
  ...
  return (
    <main className="main">
      ...
      <div className="review-layout">
        <div>
          {grouped.length ? grouped.map(({ day, sessions }) => <section className="card day-group" key={day}><h3>{new Date(`${day}T12:00:00`).toLocaleDateString([], { weekday: "long", month: "long", day: "numeric" })}</h3>{sessions.map((session) => <div className="review-session" key={session.id}><input type="checkbox" checked={app.selectedSessionIds.includes(session.id)} disabled={session.status === "synced"} aria-label={`Approve ${session.title}`} onChange={(event) => toggle(session.id, event.target.checked)} /><div className="review-time">{new Date(session.startAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}-{new Date(session.endAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</div><div><strong>{session.title}</strong><span>{session.durationMinutes} min · {session.reason}</span></div><button className="btn btn-ghost btn-sm" onClick={() => app.openModal({ type: "schedule", assignmentId: session.assignmentId })}>Edit</button></div>)}</section>) : <section className="card card-pad"><EmptyState title="No draft sessions" copy="Build a study plan to review CatchUp draft sessions here." /></section>}
          {app.data.failures.map((failure) => <section className="issue-box" key={`${failure.assignmentId}-${failure.code ?? "issue"}`}><strong>{failure.title ?? failure.assignmentId}{failure.code ? ` · ${failure.code}` : ""}</strong><p>{failure.reason}</p>{failure.unscheduledMinutes ? <p>{failure.unscheduledMinutes} minutes still need a plan.</p> : null}{failure.suggestedActions?.length ? <div className="reason-list compact-list">{failure.suggestedActions.map((action) => <div className="reason-item" key={action}><div className="reason-check">•</div><div><span>{action}</span></div></div>)}</div> : null}<button className="btn btn-secondary btn-sm" onClick={() => app.openModal({ type: "estimate", assignmentId: failure.assignmentId })}>Review estimate</button></section>)}
        </div>
