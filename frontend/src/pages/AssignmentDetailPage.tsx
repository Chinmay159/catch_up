...
export function AssignmentDetailPage({ app }: { app: AppContextValue }) {
  ...
  const assignment = app.data.assignments.find((item) => item.id === assignmentId);
  if (!assignment) {
    return (
      <main className="main">
        <section className="card card-pad">
          <EmptyState title="Assignment not found" copy="Active Classroom assignments will appear here after sync." action={<Link className="btn btn-secondary" to="/assignments">Back to assignments</Link>} />
        </section>
      </main>
    );
  }
  ...
  const progressPercent = assignment.estimatedMinutes ? Math.min(100, (scheduled / assignment.estimatedMinutes) * 100) : 0;
      ...
      <div className="detail-layout">
        <div className="dashboard-main">
          <section className="card detail-header">
            ...
            <div className="hero-meta"><div className="meta-item">{assignment.dueLabel}</div><div className="meta-item">{formatEstimateLong(assignment.estimatedMinutes)}</div><div className="meta-item"><span className="badge badge-neutral">{assignment.status}</span></div></div>
          ...
          <section className="card card-pad">
            <div className="section-title"><h3>Study plan</h3><small>{assignment.estimatedMinutes === null ? `${scheduled} minutes scheduled · estimate needed` : `${scheduled} of ${assignment.estimatedMinutes} minutes scheduled`}</small></div>
